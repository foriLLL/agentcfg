...
test('readme documents the required agentcfg MVP sections', () => {
  ...
  assert.ok(readme.includes('Treat the Web UI as trusted-local only.'));
  assert.ok(readme.includes('private Gist stores provider and agent API keys in plain text'));
  ...
  assert.ok(readme.includes('confirm with a checkbox, and apply'));
  assert.ok(readme.includes('instead of typing `APPLY`'));
  assert.ok(readme.includes('raw native config editor'));
  assert.ok(readme.includes('Provider API keys are masked in the high-level status and dashboard UI'));
  assert.ok(readme.includes('Saved GitHub Tokens are never exposed in full'));
