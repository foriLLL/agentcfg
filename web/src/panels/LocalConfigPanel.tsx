...
const CONFIG_TARGET_OPTIONS: Array<{
  value: AgentName;
  title: string;
  copy: string;
}> = [
...
export function LocalConfigPanel(props: LocalConfigPanelProps) {
  const selectedConfigTarget =
    props.configAgent === null
      ? undefined
      : CONFIG_TARGET_OPTIONS.find(
          (target) => target.value === props.configAgent,
        );
  const draftBadge = configDraftBadge({
    loaded: props.configFile !== null,
    dirty:
      props.configFile !== null &&
      props.configDraft !== props.configFile.content,
  });
  ...
  return (
    ...
    >
      ...
      <article className="card config-editor-card">
        ...
        <div
          className="config-agent-tabs"
          role="tablist"
          aria-label="选择本地配置 Agent"
        >
          {CONFIG_TARGET_OPTIONS.map((target) => {
            const availability = props.configAvailabilityByAgent.get(
              target.value,
            );
        ...
        <div className="config-editor-toolbar">
          <div className="path-form">
            ...
            <div className="path-note">
              留空时使用检测到的默认原生配置；仅当所选代理的原生配置在其他文件或目录时填写。该值会同时作为
              dry-run、应用的路径覆盖。
            </div>
            <div className="review-actions" aria-label="配置文件操作">
              <button
                ...
                type="button"
                onClick={props.onLoadConfigFile}
                disabled={!props.canLoadConfig}
              >
              ...
              <button
                ...
                type="button"
                onClick={props.onSaveConfigFile}
                disabled={!props.canSaveConfig}
              >
            ...
            <div className="local-sync-panel" aria-label="本地配置同步与应用">
              ...
              <div
                ...
                aria-label="本地配置 dry-run 与应用操作"
              >
                <button
                  className="secondary-action"
                  type="button"
                  onClick={props.onPlan}
                  disabled={!props.canReviewLocalConfig}
                >
              ...
              <div className="apply-lock" aria-label="本地配置应用安全门禁">
                ...
                <label
                  ...
                  htmlFor="local-apply-confirmation"
                >
                  <input
                ...
                <button
                  ...
                  disabled={!props.canApplyLocalConfig}
                >
        ...
        <div className="config-editor-meta" role="status" aria-live="polite">
          ...
          {props.configFile !== null && (
            <strong>{props.configFile.path}</strong>
          )}
        </div>
        ...
        <section
          className="review-results config-review-results"
          aria-label="本地配置 dry-run 与应用结果"
        >
          <PlanResults
